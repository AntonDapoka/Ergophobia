...
{
    static InspectorScript _instance;
    public static InspectorScript Instance
    {
        get
        {
            if (!_instance) _instance = FindAnyObjectByType<InspectorScript>();
            return _instance;
        }
        set => _instance = value;
    }
    ...
    public RenderMode CanvasRenderMode
    {
        ...
        set
        {
            CanvasScript[] be2CanvasArray = FindObjectsByType<CanvasScript>();
            foreach (CanvasScript be2Canvas in be2CanvasArray)
            {
...
/*
    ...
    public Transform BuildAndInstantiateBlock(string instructionName)
    {
        ...
        else if (blockType == BlockType.trigger)
        {
        ...
        else if (blockType == BlockType.operation)
        {
        ...
        ICodeBlock newBlock = newBlockGO.GetComponent<ICodeBlock>();
        IBlockLayout newBlockLayout = newBlockGO.GetComponent<IBlockLayout>();

        // v2.1 - bugfix: fixed assigned wrong Type when building Loop Blocks
        if (blockType == BlockType.loop)
        {
            newBlock.Type = BlockType.loop;
        }
        ...
        for (int h = 0; h < headersMarkup.Length; h++)
        {
            ...
            GameObject body;
            if (blockType == BlockType.condition || blockType == BlockType.loop)
            {
        ...
        newBlockGO.GetComponent<IBlockLayout>().UpdateLayout();
