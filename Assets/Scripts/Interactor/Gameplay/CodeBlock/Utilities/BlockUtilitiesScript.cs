...
public static class BlockUtilitiesScript
{/*
    public static void RemoveEngineComponents(Transform blockTransform)
    {
        ...
        IBlockSection[] sections = blockTransform.GetComponentsInChildren<I_BE2_BlockSection>();
        ...
        ISpot[] spots = blockTransform.GetComponentsInChildren<I_BE2_Spot>();
        for (int i = spots.Length - 1; i >= 0; i--) 
            UnityEngine.Object.DestroyImmediate(spots[i] as MonoBehaviour);

        if (blockTransform.TryGetComponent<IDrag>(out var drag)) 
            UnityEngine.Object.DestroyImmediate(drag as MonoBehaviour);

        if (blockTransform.TryGetComponent<IInstruction>(out var instruction))
            UnityEngine.Object.DestroyImmediate(instruction as MonoBehaviour);

        IBlockLayout layout = blockTransform.GetComponent<IBlockLayout>();
        if (layout != null) 
            UnityEngine.Object.DestroyImmediate(layout as MonoBehaviour);

        if (blockTransform.TryGetComponent<ICodeBlock>(out var block)) 
            UnityEngine.Object.DestroyImmediate(block as MonoBehaviour);
    ...
    public static void DuplicateBlock(ICodeBlock block)
    {
        IProgrammingEnvironment programmingEnv = block.Transform.GetComponentInParent<IProgrammingEnvironment>();
        ICodeBlock newBlock = BE2_BlocksSerializer.SerializableToBlock(BE2_BlocksSerializer.BlockToSerializable(block), programmingEnv);
        ...
        if (newBlock.Type == BlockType.trigger)
        {
    ...
    public static ICodeBlock GetRootBlock(ICodeBlock block)
    {
        ICodeBlock parentBlock = block.ParentSection?.Block;
        if (parentBlock != null)
        {
    ...
    public static void RemoveBlock(ICodeBlock block)
    {
    ...
    public static IInstruction GetParentInstructionOfType(IInstruction thisInstruction, BlockType blockType)
    {
        ...
        if (parentBody != null)
        {
            ICodeBlock parentBlock = parentBody.BlockSection.Block;
    ...
    public static List<IInstruction> GetParentInstructionOfTypeAll(IInstruction thisInstruction, BlockType blockType)
    {
        List<IInstruction> foundInstructions = new List<IInstruction>();
    ...
    public static bool BlockIsVariable(this ICodeBlock block)
    {
        Type instructionType = block.Instruction.GetType();
        Attribute.SerializeAsVariableAttribute varAttribute = (Attribute.SerializeAsVariableAttribute)Attribute.GetCustomAttribute(instructionType, typeof(Attribute.SerializeAsVariableAttribute));
    ...
    public static bool BlockIsFunction(this ICodeBlock block)
    {
    ...
    public static ICodeBlock InstantiateNoViewBlock(this ICodeBlock block)
    {
        if (block is BE2_GhostBlock)
            return null;

        ICodeBlock noViewBlock = null;
        IInstruction noViewInstruction = null;
    ...
    // v2.12 - added method to the BlockUtils to instantiate "noView" blocks adding only the needed components excluding the visual components
    public static ICodeBlock InstantiateNoViewBlock<T>(this ICodeBlock block) where T : IInstruction
    {
        if (block is BE2_GhostBlock)
            return null;
        ...
        GameObject noViewBlockGO = new GameObject(block.Transform.name + " noView", typeof(RectTransform));
        BE2_BlockNoViewLayout layout = noViewBlockGO.AddComponent<BE2_BlockNoViewLayout>();
        noViewBlock = noViewBlockGO.AddComponent<CodeBlockScript>() as ICodeBlock;
        noViewBlock.Type = block.Type;

        for (int i = 0; i < block.Layout.SectionsArray.Length; i++)
        {
            IBlockSection section = block.Layout.SectionsArray[i];
        ...
        if (noViewBlock.Type == BlockType.operation)
            noViewBlockGO.AddComponent<BE2_BlockSectionHeader_Operation>();
